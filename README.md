...
Create a Render Node web service from this repository with build command `npm ci --include=dev && npm run build` (`NODE_ENV=production` makes npm skip devDependencies such as Vite) and start command `npm run start --workspace server`. Set `NODE_ENV=production`, `MONGODB_URI` and `MONGODB_DB` for a dedicated persistent demo database, and `CLIENT_ORIGIN` to the exact Vercel origin, for example `https://your-site.vercel.app`. Render supplies `PORT`; leave `HOST` unset. To enable the DLAO Officer and UDC Operator quick-fill buttons on the live site, set `DEMO_CREDENTIALS` to JSON holding the fictional `demo.officer` and `demo.udc` passwords; anyone with the URL can then sign in as those roles, so use fictional data only. Check `/health` after deployment. Keep all secrets in Render's environment settings, never in Vercel's `VITE_` variables or Git.
