...
2. After the greeting, press **২** for "no one is in danger" (pressing **১** instead stops questioning, plays the 999 advice, and takes only callback details). Press **২** again: calling for someone else.
3. For spoken questions, speak after the beep and press **#** (or choose **লিখে উত্তর দিন** to type). Give fictional Ripon as caller and brother, fictional Moyuri as applicant, press **২** for identity unavailable, then the fictional account and district. Press **\*** at any point to hear a question again. The automated test covers this by keyboard only, not as a verified blind-user or screen-reader journey.
4. Choose a safe contact route on the keypad; for a phone route, dial the fictional number and press **#**. Do not promote the representative's contact to the applicant.
5. At read-back, correct a field, then press **১** to submit. Note the one-time Application ID; there is no Case ID yet.
