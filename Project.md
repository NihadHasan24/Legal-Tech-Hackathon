...
> **Project decision (2026-09-24, user-approved): 16699 call v2.** The call opens with a choice: a legal complaint or legal information/advice. Advice takes the question, a safe number, and a safe time, then waits for a helpline callback; the helpline officer closes it (information given) or records the applicant, which turns the same record into a complaint for DLAO review. A complaint asks self or representative, full name(s), district, whether the NID is known and then the NID (optional; an unknown NID plays UDC verification advice and the call continues), the complaint in the caller's words (what, when, where, who), and the current safety risk. **A reported danger no longer stops questioning:** it plays the 999 advice, flags an urgent safety alert for officer review, and the full intake continues. Contact is by phone, a UDC office (in person), or a trusted person on their own phone; SMS and voicemail are never used. The call ends by reading out the Application ID and a 6-digit status PIN (a Case ID still comes only from DLAO acceptance). The NID is format-checked only, never checked against an identity register, and is kept out of the audit trail; prototype data stays fictional. The optional AI may outline the complaint (what/when/where/who, a suggested type and legal need) as unverified suggestions for the officer; it decides nothing.
