...
export default function MediationPanel({ applicationId, session, role }) {
  ...
  useEffect(() => {
    ...
    listSignaturePackets(ownerId).then(setQueue).catch(() => setError(bi('Offline signature queue is unavailable on this device.', 'এই ডিভাইসে অফলাইন স্বাক্ষরের সারি নেই।')))
  ...
  const syncPending = useCallback(async () => {
    ...
    try {
      ...
      await refreshQueue()
    } catch (failure) { setError(failure.message || bi('Signature sync is waiting for a connection.', 'স্বাক্ষর সিঙ্ক সংযোগের অপেক্ষায়।')) }
      ...
      if (!navigator.onLine) {
        ...
        try {
          ...
          if (!failure.status || failure.status >= 500) {
            await saveSignaturePacket({ id: packet.clientMutationId, ownerId, value: packet, passphrase: signingPassphrase })
            await refreshQueue()
            setNotice(bi('Connection failed after signing. The encrypted signature is queued for retry.', 'স্বাক্ষরের পর সংযোগ ব্যর্থ। এনক্রিপ্ট করা স্বাক্ষর আবার চেষ্টার সারিতে আছে।'))
          } else throw failure
        }
      }
  ...
  const verifier = <Link to={`/applications/${applicationId}/mediation/verify`}><Bi en="Open independent signature verifier" bn="স্বাধীন স্বাক্ষর যাচাই খুলুন" /></Link>
  ...
  return <Panel id="mediation-title" en="Mediation" bn="মধ্যস্থতা" hint={loaded ? mediation ? say(mediation.stage) : bi('Not started', 'শুরু হয়নি') : undefined} open={role !== 'DLAO_OFFICER'}>
    ...
    {loaded && <p className="muted"><Bi en="Records what people do. Sends no notices and decides no legal outcome." bn="মানুষের কাজ রেকর্ড করে। নোটিশ পাঠায় না, আইনি ফল ঠিক করে না।" /></p>}
    ...
    {mediation && <>
      ...
      {mediatorCanAct && ['REGISTRATION', 'SCHEDULING_NOTICES'].includes(mediation.stage) && <form className="form-stack inline-form" onSubmit={(event) => {
        ...
        send('/schedule', { mode, scheduledAt: new Date(scheduledAt).toISOString(), venue, inPersonFallback, notices: ['PARTY_A', 'PARTY_B'].map((party) => ({ party, ...notices[party] })) }, bi('Schedule and notices saved. Nothing was sent.', 'সময় ও নোটিশ সংরক্ষিত। কিছু পাঠানো হয়নি।'))
      }}>
        ...
        {mode !== 'IN_PERSON' && <><label htmlFor="mediation-fallback"><Bi en="In-person backup plan" bn="সরাসরি সভার বিকল্প পরিকল্পনা" /></label><textarea id="mediation-fallback" value={inPersonFallback} onChange={(event) => setInPersonFallback(event.target.value)} minLength="10" maxLength="300" required /></>}
      ...
      {mediatorCanAct && mediation.stage === 'MEDIATION' && <>
        ...
        {mediation.outcome === 'AGREEMENT_REACHED' && <form className="form-stack inline-form" onSubmit={(event) => { event.preventDefault(); send('/draft', { template, notes, identifiersRemoved }, bi('Draft ready. It is not final; review it.', 'খসড়া তৈরি। এটি চূড়ান্ত নয়; পর্যালোচনা করুন।')) }}>
          <h3><Bi en="Draft the settlement" bn="মীমাংসার খসড়া" /></h3><p className="muted"><Bi en="Anonymised notes only; they may go to the AI service and are not stored." bn="শুধু নামহীন নোট; এআই সেবায় যেতে পারে, সংরক্ষণ হয় না।" /></p><label htmlFor="settlement-template"><Bi en="Template" bn="নমুনা" /></label><select id="settlement-template" value={template} onChange={(event) => setTemplate(event.target.value)}>{['MAINTENANCE', 'PROPERTY', 'LABOUR'].map((code) => <option key={code} value={code}>{say(code)}</option>)}</select><label htmlFor="mediator-notes"><Bi en="Anonymised notes" bn="নামহীন নোট" /></label><textarea id="mediator-notes" value={notes} onChange={(event) => setNotes(event.target.value)} minLength="10" maxLength="3000" required /><label className="checkbox-label" htmlFor="identifiers-removed"><input id="identifiers-removed" type="checkbox" checked={identifiersRemoved} onChange={(event) => setIdentifiersRemoved(event.target.checked)} required /><Bi en="I removed names, phone numbers, addresses and ID numbers." bn="নাম, ফোন নম্বর, ঠিকানা ও পরিচয় নম্বর মুছে দিয়েছি।" /></label><button type="submit" disabled={busy || !identifiersRemoved}><Bi en="Prepare draft" bn="খসড়া তৈরি করুন" /></button>
        </form>}
      </>}
      ...
      {mediatorCanAct && mediation.stage === 'DRAFT_OUTCOME' && mediation.draft && <section className="form-stack inline-form" aria-labelledby="settlement-title">
        ...
        <p className="safety-note"><Bi en="Check every amount, date and duty. AI can miss contradictions." bn="প্রতিটি অঙ্ক, তারিখ ও দায়িত্ব যাচাই করুন। এআই ভুল এড়িয়ে যেতে পারে।" /></p>
        ...
        {mediation.draft.status === 'HUMAN_REVIEW' && <>
          ...
          <h4><Bi en="Did the parties understand and agree?" bn="পক্ষরা কি বুঝেছেন ও সম্মত?" /></h4><p className="muted"><Bi en="Your attestation, not proof of identity or capacity." bn="এটি আপনার সাক্ষ্য, পরিচয় বা সক্ষমতার প্রমাণ নয়।" /></p>
      ...
      {signing && <section className="form-stack inline-form" aria-labelledby="signature-title">
        ...
        <p className="muted"><Bi en="The signing key is made in this browser and thrown away. Offline packets hold no document text." bn="স্বাক্ষরের চাবি এই ব্রাউজারে তৈরি হয়ে মুছে যায়। অফলাইন প্যাকেটে নথির লেখা থাকে না।" /></p>
      ...
      {role === 'CLAO' && mediation.stage === 'PENDING_CLAO_CERTIFICATION' && <section className="form-stack inline-form" aria-labelledby="clao-title">
        ...
        {mediation.legalApplicability !== 'APPLICABLE_VERIFIED' ? <form className="form-stack" onSubmit={(event) => { event.preventDefault(); send('/legal-applicability', { applicability: 'APPLICABLE_VERIFIED', basis: applicabilityBasis }, bi('Applicability recorded. Certification is a separate step.', 'প্রযোজ্যতা লেখা হয়েছে। সনদ আলাদা ধাপ।')) }}><label htmlFor="legal-basis"><Bi en="Verified legal basis" bn="যাচাইকৃত আইনি ভিত্তি" /></label><textarea id="legal-basis" value={applicabilityBasis} onChange={(event) => setApplicabilityBasis(event.target.value)} minLength="10" maxLength="500" required /><button type="submit" disabled={busy}><Bi en="Record verified applicability" bn="প্রযোজ্যতা লিখুন" /></button><button type="button" className="secondary-button" disabled={busy} onClick={() => send('/legal-applicability', { applicability: 'UNVERIFIED' }, bi('Left unverified; final status is blocked.', 'যাচাইহীন রাখা হলো; চূড়ান্ত অবস্থা বন্ধ।'))}><Bi en="Leave unverified" bn="যাচাইহীন রাখুন" /></button></form> : <><p><Bi en="Legal basis:" bn="আইনি ভিত্তি:" /> {mediation.legalReviewBasis}</p><form className="form-stack" onSubmit={(event) => { event.preventDefault(); send('/certify', { reason: certificateReason }, bi('CLAO certification recorded. No court-decree finding is made.', 'সিএলএও সনদ লেখা হয়েছে। আদালতের ডিক্রির সিদ্ধান্ত নয়।')) }}><label htmlFor="certificate-reason"><Bi en="Certification reason" bn="সনদের কারণ" /></label><textarea id="certificate-reason" value={certificateReason} onChange={(event) => setCertificateReason(event.target.value)} minLength="10" maxLength="500" required /><button type="submit" disabled={busy}><Bi en="Record CLAO certification" bn="সনদ লিখুন" /></button></form></>}
      </section>}

      {mediation.stage === 'CERTIFIED_FINAL' && <p role="status" className="safety-note"><Bi en="CLAO certification recorded. Court-decree status is not decided here." bn="সিএলএও সনদ লেখা হয়েছে। আদালতের ডিক্রির অবস্থা এখানে ঠিক হয় না।" /></p>}
