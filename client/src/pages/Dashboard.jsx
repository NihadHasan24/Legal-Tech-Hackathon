...
const roles = {
  DLAO_OFFICER: [['DLAO officer', 'ডিএলএও কর্মকর্তা'], ['Review the shared queue and make recorded human decisions.', 'যৌথ তালিকা দেখুন এবং রেকর্ডসহ সিদ্ধান্ত দিন।']],
  CASE_SUPPORT: [['Case support', 'মামলা সহায়তা'], ['Find shared records and reconstruct routine follow-up.', 'রেকর্ড খুঁজুন এবং নিয়মিত ফলো-আপ দেখুন।']],
  ...
  UDC_OPERATOR: [['UDC operator', 'ইউডিসি অপারেটর'], ['Submit a bounded assisted intake. Offline tools come later.', 'সহায়তায় আবেদন জমা দিন। অফলাইন টুল পরে আসবে।']],
...
export default function Dashboard({ session }) {
  ...
  return <section aria-labelledby="dashboard-title">
    <p className="eyebrow">{workspace?.officeCode || bi('Demo office', 'ডেমো অফিস')} · {bi('Shared record', 'যৌথ রেকর্ড')}</p>
    ...
    {submitted && <p role="status" className="success">{bi(`Application ${submitted.applicationId} submitted to the DLAO queue. No Case ID exists yet. Give this lookup code to the caller once, through the agreed safe route:`, `আবেদন ${submitted.applicationId} ডিএলএও তালিকায় জমা হয়েছে। এখনো মামলা নম্বর নেই। সম্মত নিরাপদ পথে কলারকে এই কোডটি একবার দিন:`)} <code>{submitted.lookupCode}</code>. {bi('It will not be shown again.', 'এটি আর দেখানো হবে না।')}</p>}
    ...
    {role === 'HELPLINE_AGENT' && <section className="card" aria-labelledby="lookup-title"><h2 id="lookup-title">{bi('Status lookup', 'অবস্থা জানুন')}</h2><p className="muted">{bi('Ask for the ID and one-time code, then do the approved caller check. The code alone is not proof of identity. Nothing is sent to the number on file.', 'নম্বর ও এককালীন কোড চেয়ে নিন, তারপর অনুমোদিত উপায়ে কলার যাচাই করুন। শুধু কোড পরিচয়ের প্রমাণ নয়। লেখা নম্বরে কিছু পাঠানো হয় না।')}</p><form onSubmit={statusLookup} className="form-stack inline-form"><label htmlFor="lookup-id">{bi('Application or Case ID', 'আবেদন বা মামলা নম্বর')}</label><input id="lookup-id" value={identifier} onChange={(event) => setIdentifier(event.target.value)} required /><label htmlFor="lookup-code">{bi('One-time lookup code', 'এককালীন কোড')}</label><input id="lookup-code" value={lookupCode} onChange={(event) => setLookupCode(event.target.value)} minLength="24" maxLength="24" autoComplete="off" required /><label htmlFor="status-channel">{bi('Permitted lookup route', 'অনুমোদিত পথ')}</label><select id="status-channel" value={contactChannel} onChange={(event) => setContactChannel(event.target.value)}><option value="PHONE">{say('PHONE')}</option><option value="IN_PERSON">{say('IN_PERSON')}</option></select><label className="checkbox-label" htmlFor="caller-verified"><input id="caller-verified" type="checkbox" checked={callerVerified} onChange={(event) => setCallerVerified(event.target.checked)} required />{bi('I performed the approved human caller-verification procedure.', 'আমি অনুমোদিত উপায়ে কলার যাচাই করেছি।')}</label><button type="submit">{bi('Check permitted status', 'অবস্থা দেখুন')}</button></form>{lookup && <div role="status" className="success"><p>{lookup.applicationId} · {say(lookup.status)}{lookup.caseId ? ` · ${lookup.caseId}` : ''}</p><p>{bi('Next hearing:', 'পরবর্তী শুনানি:')} {lookup.nextHearingAt ? <time dateTime={lookup.nextHearingAt}>{when(lookup.nextHearingAt)}</time> : bi('Not recorded', 'লেখা নেই')}</p><p>{bi('Next step:', 'পরবর্তী ধাপ:')} {tr(lookup.nextAction || lookup.nextStep)}</p><p className="muted">{bi('Read this status to the caller. Nothing was sent.', 'কলারকে এই অবস্থা পড়ে শোনান। কিছু পাঠানো হয়নি।')}</p></div>}{verifiedLookup && lookup?.status === 'ACCEPTED' && <form onSubmit={requestLawyerChange} className="form-stack inline-form"><h3>{bi('Record a lawyer-change request', 'আইনজীবী বদলের অনুরোধ লিখুন')}</h3><p className="muted">{bi('For DLAO review only. The lawyer does not change here.', 'শুধু ডিএলএও পর্যালোচনার জন্য। এখানে আইনজীবী বদলায় না।')}</p><label htmlFor="lawyer-change-reason">{bi('Applicant’s stated reason', 'আবেদনকারীর বলা কারণ')}</label><textarea id="lawyer-change-reason" value={lawyerChangeReason} onChange={(event) => setLawyerChangeReason(event.target.value)} minLength="5" maxLength="1000" required /><button type="submit">{bi('Send request to DLAO review', 'ডিএলএও পর্যালোচনায় পাঠান')}</button></form>}{lawyerChangeNotice && <p role="status" className="success">{tr(lawyerChangeNotice)}</p>}</section>}
    ...
    <section className="worklist" aria-labelledby="worklist-title"><div className="section-heading"><h2 id="worklist-title">{staff ? bi('Daily queue and history', 'দৈনিক তালিকা ও ইতিহাস') : role === 'PANEL_LAWYER' ? bi('Assigned cases', 'নিযুক্ত মামলা') : role === 'RECEIVING_DLAO' ? bi('Referrals to your office', 'আপনার অফিসে আসা রেফারেল') : bi('Role worklist', 'কাজের তালিকা')}</h2><span className="muted">{bi(`${workspace?.records.length || 0} records`, `${num(workspace?.records.length || 0)}টি রেকর্ড`)}</span></div>
      ...
      {!loading && workspace?.records.length === 0 && <p className="empty-state">{role === 'PANEL_LAWYER' ? bi('No current or pending panel-lawyer assignments.', 'বর্তমান বা অপেক্ষমাণ কোনো নিয়োগ নেই।') : bi('No records are available to this role yet.', 'এই ভূমিকার জন্য এখনো কোনো রেকর্ড নেই।')}</p>}
      {!loading && workspace?.records.length > 0 && visible.length === 0 && <p className="empty-state">{bi('No records match these filters.', 'এই বাছাইয়ে কোনো রেকর্ড মেলেনি।')}</p>}
      {!loading && visible.length > 0 && <ul className="record-list">{visible.map((record) => <li key={record.referralId ?? record.assignmentId ?? record.applicationId}>{role === 'PANEL_LAWYER'
