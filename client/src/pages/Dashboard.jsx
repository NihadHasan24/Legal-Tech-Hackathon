...
export default function Dashboard({ session }) {
  ...
  return <section aria-labelledby="dashboard-title">
    ...
    <section className="worklist" aria-labelledby="worklist-title"><div className="section-heading"><h2 id="worklist-title">{staff ? bi('Daily queue and history', 'আজকের কাজ ও আগের রেকর্ড') : role === 'PANEL_LAWYER' ? bi('Assigned cases', 'নিযুক্ত মামলা') : role === 'RECEIVING_DLAO' ? bi('Referrals to your office', 'আপনার অফিসে আসা রেফারেল') : bi('Role worklist', 'কাজের তালিকা')}</h2><span className="muted">{bi(`${workspace?.records.length || 0} records`, `${num(workspace?.records.length || 0)}টি রেকর্ড`)}</span></div>
      ...
      {!loading && workspace?.records.length === 0 && <p className="empty-state">{role === 'PANEL_LAWYER' ? bi('No current or pending panel-lawyer assignments.', 'বর্তমান বা অপেক্ষমাণ কোনো নিয়োগ নেই।') : bi('No records are available to this role yet.', 'আপনার কাজের তালিকায় এখনো কোনো রেকর্ড নেই।')}</p>}
